...
import (
	"context"
	dpfm_api_caller "data-platform-api-fin-inst-account-creates-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-fin-inst-account-creates-rmq-kube/existence_conf"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncFinInstAccountCreates(accepter, &input, &output, l)
	if len(errs) != 0 {
