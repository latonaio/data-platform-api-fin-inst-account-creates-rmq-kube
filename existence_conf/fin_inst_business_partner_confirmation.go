...
func (c *ExistenceConf) headerFinInstBusinessPartnerExistenceConf(input *dpfm_api_input_reader.SDC, existenceMap *[]bool, exconfErrMsg *string, errs *[]error, mtx *sync.Mutex, wg *sync.WaitGroup, log *logger.Logger) {
	defer wg.Done()
	if input.Header.FinInstBusinessPartner == nil {
		*exconfErrMsg = "cannot specify null keys"
		return
	}
	res, err := c.bpGeneralExistenceConfRequest(*input.Header.FinInstBusinessPartner, input, existenceMap, mtx, log)
	if err != nil {
		mtx.Lock()
