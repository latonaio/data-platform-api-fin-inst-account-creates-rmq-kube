...
import (
	...
	dpfm_api_output_formatter "data-platform-api-fin-inst-account-creates-rmq-kube/DPFM_API_Output_Formatter"
	"data-platform-api-fin-inst-account-creates-rmq-kube/config"
	"encoding/json"
	"fmt"
	"strings"
	"sync"
...
func (c *ExistenceConf) Conf(data *dpfm_api_input_reader.SDC, ssdc *dpfm_api_output_formatter.SDC, accepter []string, l *logger.Logger) (allExist bool, errs []error) {
	...
	for _, v := range *exConfMapper {
		...
		case "FinInstCountry":
			wg.Add(1)
			go c.finInstCountryExistenceConf(v, data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
		case "FinInstCode":
			wg.Add(1)
			go c.finInstCodeExistenceConf(v, data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
		case "BusinessPartnerGeneral":
			switch apiName {
			case "BusinessPartner":
				wg.Add(1)
				go c.headerFinInstBusinessPartnerExistenceConf(v, data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
			}
		}
	}
